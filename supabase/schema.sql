...
-- profiles: everyone signed in can read (needed to show names); only admin can change roles;
...
-- products / suppliers / customers: readable by anyone signed in, editable by staff+admin
create policy "products_select" on products for select to authenticated using (true);
create policy "products_write" on products for insert to authenticated with check (is_staff_or_admin());
create policy "products_update" on products for update to authenticated using (is_staff_or_admin());
...
-- transactions / transaction_items: readable by anyone signed in, writable by staff+admin
...
-- stock_adjustments: admin only, both read and write
create policy "stock_adjustments_select" on stock_adjustments for select to authenticated using (is_admin());
create policy "stock_adjustments_write" on stock_adjustments for insert to authenticated with check (is_admin());

-- ============================================================
-- After running this file:
-- 1. Go to Authentication -> Users, invite/create your first login.
-- 2. In SQL Editor run:
--      update profiles set role = 'admin' where id = '<the new user''s UUID>';
--    to make that account an Admin (needed for Adjustment + Users pages).
-- ============================================================
